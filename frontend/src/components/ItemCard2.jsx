...
function OwnerAvatar({ item, name }) {
  ...
  return (
    <span className="h-6 w-6 shrink-0 overflow-hidden rounded-full bg-gradient-to-br from-[#E8F4EF] via-[#DDF1EA] to-[#F5EFE4] text-[#4C7468] shadow-[0_4px_10px_rgba(15,23,42,0.06)]">
      {photo ? (
        <img src={photo} alt={name} className="h-full w-full object-cover contrast-[1.03] brightness-[1.01] saturate-[1.02]" referrerPolicy="no-referrer" />
      ) : (
...
export default function ItemCard({
  ...
  const content = (
    ...
    >
      <div className="relative aspect-[1.05/1] overflow-hidden bg-[#F5F7F5]">
        <img
          src={image}
          alt={title}
          className="h-full w-full object-cover contrast-[1.03] brightness-[1.01] saturate-[1.02] transition duration-500 group-hover:scale-[1.018]"
        />
