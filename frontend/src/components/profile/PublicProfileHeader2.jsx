...
function ProfileAvatar({ profile }) {
  ...
  return (
    <div className="relative h-[96px] w-[96px] shrink-0 overflow-hidden rounded-full border-[5px] border-white bg-gradient-to-br from-[#E8F4EF] via-[#DDF1EA] to-[#F6EFE5] shadow-[0_22px_46px_rgba(15,23,42,0.10)]">
      {photo ? (
        <img
          src={photo}
        ...
        <div className="flex h-full w-full items-center justify-center bg-[radial-gradient(circle_at_35%_25%,#ffffff_0%,#E6F4EE_42%,#F4EFE4_100%)] text-[34px] font-black text-[#4C7468]">
          {getInitial(profile)}
        </div>
      )}

      <span className="absolute bottom-2 right-2 flex h-7 w-7 items-center justify-center rounded-full border-[4px] border-white bg-[#67C99E] text-white shadow-[0_8px_18px_rgba(103,201,158,0.28)]">
...
function MicroInfo({ icon: Icon, strong, muted }) {
  return (
    <span className="inline-flex items-center gap-1.5 text-[12px] font-semibold text-[#65747B]">
      <Icon size={14} strokeWidth={2.15} className="text-[#4B9D8D]" />
...
function ReputationBadges({ feedbacks }) {
  ...
  return (
    <div className="mt-3 flex flex-wrap gap-1.5">
      ...
      {rep.total >= 5 && (
        <span className="inline-flex items-center gap-1.5 rounded-full bg-slate-50 px-2.5 py-1 text-[11px] font-black text-slate-500">
          {rep.total} avis
        </span>
      )}
    </div>
...
export default function PublicProfileHeader({ profile, userId, feedbacks = [] }) {
  ...
  return (
    <section className="relative overflow-hidden rounded-[30px] border border-[#ECF4F0] bg-white/[0.94] p-5 md:p-6 shadow-[0_20px_60px_rgba(15,23,42,0.065)]">
      ...
      <div className="relative">
        <div className="absolute right-0 top-0 flex justify-end">
          <button
            type="button"
            className="flex h-9 w-9 items-center justify-center rounded-full bg-white/90 text-[#081225] shadow-[0_8px_24px_rgba(15,23,42,0.045)]"
            aria-label="Plus d’options"
          >
            <MoreHorizontal size={18} strokeWidth={2.35} />
          </button>
        </div>

        <div className="grid gap-4 pr-10 lg:grid-cols-[112px_1fr] lg:items-center">
          <ProfileAvatar profile={profile} />

          <div className="min-w-0">
            <h1 className="text-[34px] font-black leading-[0.94] tracking-[-0.06em] text-[#081225]">
              {getDisplayName(profile)}
            </h1>

            <p className="mt-1 text-[14px] font-black text-[#2F9A82]">
              {getHandle(profile, userId)}
            </p>

            <p className="mt-2 flex items-center gap-2 text-[14px] font-bold text-[#60737A]">
              <MapPin size={17} className="text-[#4B9D8D]" />
              {getLocation(profile)}
            </p>

            <p className="mt-3 max-w-[540px] text-[15px] font-medium leading-relaxed text-[#283A46]">
              {getBio(profile)}
            </p>
        ...
        <div className="mt-4 flex flex-wrap items-center gap-x-4 gap-y-2 rounded-[22px] border border-white/80 bg-white/82 px-4 py-3 shadow-[0_8px_24px_rgba(15,23,42,0.03)]">
