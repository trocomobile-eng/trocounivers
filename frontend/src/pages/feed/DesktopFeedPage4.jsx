...
function DesktopSidebar({ user }) {
  ...
  return (
    <aside className="fixed left-0 top-0 hidden h-screen w-[252px] border-r border-white/70 bg-white/48 px-6 py-8 backdrop-blur-2xl lg:flex lg:flex-col">
...
function HeroSearch({ search, setSearch }) {
  return (
    <div className="flex h-13 w-full max-w-[620px] items-center gap-3 rounded-[22px] border border-white/75 bg-white/92 px-4 py-3 shadow-[0_12px_30px_rgba(15,23,42,0.08)] backdrop-blur-xl">
      <Search size={18} className="shrink-0 text-slate-500" strokeWidth={2.2} />
      <input
        value={search}
        onChange={(event) => setSearch(event.target.value)}
        placeholder="Rechercher un objet, un univers, une personne..."
        className="min-w-0 flex-1 bg-transparent text-[14px] font-medium text-slate-800 outline-none placeholder:text-slate-500"
      />
      <button
        type="button"
...
function HeroStat({ icon, title, subtitle }) {
  return (
    <div className="flex items-center gap-3 border-r border-slate-200/70 px-5 last:border-r-0">
      <span className="text-[#167a58]">{icon}</span>
      <div>
      ...
      <div className="relative z-10 min-h-[450px] px-10 pb-9 pt-12">
        <div className="flex items-start justify-between gap-6">
          ...
          <div className="flex shrink-0 items-center gap-3">
            <button className="relative flex h-11 w-11 items-center justify-center rounded-full bg-white/82 text-slate-800 shadow-[0_10px_24px_rgba(15,23,42,0.08)] backdrop-blur-xl">
              <Bell size={17} />
              <span className="absolute right-2 top-2 h-2.5 w-2.5 rounded-full border border-white bg-[#167a58]" />
            </button>
            <button className="flex h-11 w-11 items-center justify-center rounded-full bg-white/82 text-slate-800 shadow-[0_10px_24px_rgba(15,23,42,0.08)] backdrop-blur-xl">
              <Heart size={18} />
            </button>
          </div>
        </div>
        ...
        <div className="mt-10 max-w-[720px]">
          <p className="mb-3 text-[12px] font-black uppercase tracking-[0.18em] text-[#167a58]">
            Troco local
          </p>
          <h1 className="max-w-[600px] text-[58px] font-black leading-[0.92] tracking-[-0.07em] text-[#102033]">
            Échange local, impact réel.
          </h1>
          <p className="mt-5 max-w-[520px] text-[18px] font-semibold leading-relaxed text-slate-700">
            Découvre des objets près de chez toi et échange autrement.
          </p>

          <div className="mt-8 flex w-fit overflow-hidden rounded-[22px] border border-white/70 bg-white/82 py-3 shadow-[0_14px_34px_rgba(15,23,42,0.075)] backdrop-blur-xl">
            <HeroStat
...
export default function DesktopFeedPage() {
  ...
  return (
    <div className="min-h-screen bg-[var(--troco-bg)] text-[#102033]">
      ...
      <main className="px-4 pb-20 pt-4 lg:px-7 lg:pt-6 xl:px-8">
