...
function UniversCard({ univers, active = false, onClick }) {
  return (
    <button
      ...
      onClick={onClick}
      className={[
        "group relative h-[150px] overflow-hidden rounded-[22px] border bg-slate-900 text-left shadow-[0_12px_30px_rgba(15,23,42,0.07)] transition hover:-translate-y-0.5 active:scale-[0.985]",
        active ? "border-[#167a58] ring-2 ring-[#167a58]/25" : "border-white/70",
      ].join(" ")}
    >
      {univers.img ? (
        <img
      ...
      {active ? (
        <span className="absolute right-3 top-3 rounded-full bg-white/92 px-2.5 py-1 text-[10px] font-black text-[#167a58] shadow-sm">
          Filtre actif
        </span>
      ) : null}
    ...
    <div className="min-h-screen bg-[var(--troco-bg)] text-[#102033]">
      ...
      <main className="px-4 pb-20 pt-4 lg:px-7 lg:pt-6 xl:px-8">
        ...
        <section className="mt-9">
          ...
          {activeUniverse ? (
            <div className="mt-4 flex items-center justify-between rounded-[22px] border border-white/80 bg-white/72 px-4 py-3 shadow-[0_10px_24px_rgba(15,23,42,0.04)] backdrop-blur-xl">
              <p className="text-[13px] font-bold text-slate-600">
                Filtre actif : <span className="text-[#167a58]">{activeUniverseLabel}</span>
              </p>
              <button
                type="button"
                onClick={() => setActiveUniverse(null)}
                className="rounded-full bg-[#167a58] px-3 py-1.5 text-[12px] font-black text-white"
              >
