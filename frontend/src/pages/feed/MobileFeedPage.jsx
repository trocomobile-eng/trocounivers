...
function ItemCard({ item, favorite, onToggleFavorite }) {
  ...
  return (
    <Link to={`/items/${item.id}`} className="block">
      <article className="bg-white rounded-[14px] overflow-hidden border border-[#ece8e0]">
        {/* photo */}
        <div className="relative aspect-square bg-[#f0ebe0] overflow-hidden">
          ...
          <button
            ...
            className={[
              "absolute top-2 right-2 h-7 w-7 flex items-center justify-center rounded-full backdrop-blur-md shadow-sm transition active:scale-95",
              favorite ? "bg-white text-rose-500" : "bg-white/70 text-slate-400",
            ].join(" ")}
            aria-label={favorite ? "Retirer des favoris" : "Ajouter aux favoris"}
          >
            <Heart size={13} strokeWidth={2.2} fill={favorite ? "currentColor" : "none"} />
          </button>
        </div>

        {/* info */}
        <div className="p-2.5">
          <h3 className="text-[12px] font-black leading-tight tracking-tight text-[#111] line-clamp-1">
            {title}
          </h3>
          <p className="mt-0.5 text-[10.5px] text-[#888] line-clamp-1">{desc}</p>
          ...
          <div className="mt-2.5 flex items-center justify-between">
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-[10.5px] font-medium text-[#555]">{owner || "Anonyme"}</span>
            </div>
            <span className="text-[10.5px] font-bold text-[#1a7a4a] bg-[#e6f5ee] px-2 py-0.5 rounded-full">
              Échange
            </span>
          </div>
        </div>
...
export default function MobileFeedPage() {
  ...
  const isAllObjectsRoute = location.pathname === "/feed/all";
  ...
  useEffect(() => {
    if (!isAllObjectsRoute) setShowAllItems(false);
  }, [search, activeUnivers, filterRadius, filterSort, isAllObjectsRoute]);
        ...
        <section>
          ...
          <div className="flex gap-2.5 overflow-x-auto pb-1 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
            ...
            >
              ...
              <span className="text-[10.5px] font-black">Tous</span>
            </button>
        ...
        <section>
          <div className="flex items-center justify-between mb-3">
            ...
            <button
              ...
              className="flex items-center gap-1 text-[10.5px] font-bold bg-white border border-[#e8e4dc] rounded-full px-2.5 py-1"
          ...
          {visibleItems.length === 0 ? (
            <div className="bg-white rounded-[14px] p-6 text-center text-[13px] text-[#aaa] font-medium border border-[#ece8e0]">
              Aucun objet trouvé dans cette zone.
            </div>
          ) : (
            <div className="grid grid-cols-3 gap-2.5">
              {((showAllItems || isAllObjectsRoute) ? visibleItems : visibleItems.slice(0, 8)).map((item) => (
                <ItemCard
                  key={item.id}
                  item={item}
          ...
          {visibleItems.length > 8 && (
            ...
            >
              Voir les {visibleItems.length} objets <ChevronRight size={13} strokeWidth={2.5} />
            </button>
          )}
        </section>
          ...
          >
            ...
            <div className="flex items-center justify-between mb-5">
              ...
              <button type="button" onClick={() => setShowFilters(false)}
                className="h-7 w-7 flex items-center justify-center rounded-full bg-slate-100">
            ...
            {/* Distance */}
            <div className="mb-5">
              <p className="text-[10.5px] font-black uppercase tracking-widest text-slate-400 mb-2.5">Distance</p>
            ...
            {/* Tri */}
            <div className="mb-6">
              <p className="text-[10.5px] font-black uppercase tracking-widest text-slate-400 mb-2.5">Trier par</p>
