...
function TagEditor({ title, description, tags, input, setInput, onAdd, onRemove, tone = "emerald" }) {
  const pillClass =
    tone === "rose" ? "bg-rose-50 text-rose-600" : "bg-[#E8F7EF] text-[#0f9f9a]";
    ...
    <MobileLayout withBottomNav={false}>
      ...
      <main className="mx-auto max-w-4xl space-y-5 px-4 pb-10">
        ...
        <TagEditor
          ...
          onAdd={addLookingFor}
          onRemove={(tag) => { setLookingFor((current) => current.filter((item) => item !== tag)); markDirty(); }}
        />
        ...
        <TagEditor
          ...
          onRemove={(tag) => { setNotLookingFor((current) => current.filter((item) => item !== tag)); markDirty(); }}
          tone="rose"
        />
