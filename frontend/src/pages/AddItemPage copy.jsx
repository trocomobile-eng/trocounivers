...
export default function AddItemPage() {
  ...
  return (
    <div className="min-h-screen pb-32 text-[#081225]">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <SectionCard>
            ...
            <div className="mt-5 space-y-4">
              <Field
                label={`Nom de l'objet (${rawTitle.length}/20)`}
hint="Ex : Lampe vintage, Guitare Yamaha, Jean Levi's"
              >
                <TextInput
                  value={rawTitle}
                  onChange={(event) => handleRawTitleChange(event.target.value.slice(0, 60))}
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <Field label={`Titre affiché ${title.length > 0 ? `(${title.length}/50)` : ""}`} hint={title.length > 45 ? "Trop long, raccourcis pour un meilleur affichage." : ""}>
                  <TextInput
                    value={title}
