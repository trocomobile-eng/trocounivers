 ...
 *  "waiting_confirmation" — j'ai confirmé le troc, j'attends l'autre
 *  "completed"            — troc terminé (les deux ont confirmé)
...
export function getExchangeStage(exchange, currentUserId) {
  ...
  // J'ai confirmé le troc, j'attends que l'autre confirme aussi
  if (status === "awaiting_other_confirmation") return "waiting_confirmation";
...
export function exchangeNeedsAttention(exchange, currentUserId) {
  ...
  return (
    ...
    // Note: "waiting_confirmation" n'est PAS une action requise — on attend l'autre
  );
}
...
export function getExchangePriority(exchange, currentUserId) {
  ...
  if (stage === "waiting_confirmation") return 4; // même priorité que confirmed
